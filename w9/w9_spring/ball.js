...
class Ball {
  ...
  updateVel(prevBall) {
    /*way1: towards the prevBall directly*/
    ...
    /*way2: sub(destination, this.vel*/
    var destination = p5.Vector.sub(prevBall.pos, this.pos);
    var force = p5.Vector.sub(destination, this.vel);
    var distance = this.pos.dist(prevBall.pos);
    var VEL_C = map(distance, 0, 100, 0.3, 1.5);
    force.normalize();
    force.mult(VEL_C);
