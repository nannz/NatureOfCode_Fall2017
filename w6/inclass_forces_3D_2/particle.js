...
class Particle {
  ...
  checkFloorWall(){
    ...
    //x
    if(this.pos.x < -FLOOR_SIZE / 2 || this.pos.x > FLOOR_SIZE / 2){//x is out of the wall.x
      this.vel.x *= -1;
      this.vel.x *= co_restitution;
    }
    //z
    if(this.pos.z < -FLOOR_SIZE / 2 || this.pos.z > FLOOR_SIZE / 2){//x is out of the wall.x
      this.vel.z *= -1;
      this.vel.z *= co_restitution;
    }
  ...
  checkCollision(other){
    ...
    if(distance < this.rad + other.rad){
      ...
      colForce.normalize();
      colForce.mult(this.vel.mag() * CO_COLLISION);
      other.applyForce(colForce);
      //other particle
      colForce.mult(-1);
      colForce.normalize();
      colForce.mult(other.vel.mag()* CO_COLLISION);
